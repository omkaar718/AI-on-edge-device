...
def main(start_n, end_n, max_identities, max_gallery, gallery_folder):
    ...
    resolutions = [8, 16, 32, 69, 89, 106, 120, 133, 144, 155, 165, 174, 183, 193, 200, 208, 215, 221, 230, 240, 252, 257, 268, 279, 289, 298, 306, 310, 322, 330, 334, 340, 350, 360, 369, 381, 391, 400, 410, 420, 431, 441, 451, 460, 470, 481, 492, 500, 510, 521, 532, 542, 552,  561, 571, 581, 591, 600]
            ...
            if data.ok:
                ...
                if(check):
                    ...
                    if(gallery_counter > max_gallery):
                        ...
                        print('probe counter : ', probe_counter)
                        print('results_in_fraction : \n',results_in_fraction)
                        with open(f'/storage/oprabhune/tianen_colab/face_roi/naive_results/iMac_naive_results_max-identities_{max_identities}_n_{start_n}-{end_n}_in_fraction.npy', 'wb') as f:
                            np.save(f, results_in_fraction)

                        with open(f'/storage/oprabhune/tianen_colab/face_roi/naive_results/iMac_naive_results_max-identities_{max_identities}_n_{start_n}-{end_n}.npy', 'wb') as f:
                            np.save(f, results)
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--start_n', type=int, default=1356)
    #parser.add_argument('--end_n', type=int, default=3417)
    parser.add_argument('--start_n', type=int)
    parser.add_argument('--end_n', type=int)
    parser.add_argument('--create_gallery', action='store_true')
    parser.add_argument('--max_identities', type=int, default=100)
